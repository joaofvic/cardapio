...
export function CitySelectionDialog({ isOpen, onOpenChange, onCitySelect }: CitySelectionDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden border-none rounded-[2rem] bg-white shadow-2xl animate-in zoom-in [animation-duration:300ms] ease-out">
        <div className="bg-primary p-10 text-center relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16" />
          <div className="absolute bottom-0 left-0 w-24 h-24 bg-white/5 rounded-full -ml-12 -mb-12" />
          
          <div className="bg-white/20 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 backdrop-blur-md animate-in zoom-in duration-300 ease-out">
        ...
        <div className="p-6">
          {!selectedTemp ? (
            <div className="grid grid-cols-1 gap-2 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out">
              {CITIES.map((city) => (
                <button
                  key={city}
            ...
            <div className="py-4 animate-in zoom-in-95 fade-in duration-300 ease-out">
              <div className="bg-amber-50 border border-amber-200 p-6 rounded-[2rem] mb-6 flex flex-col items-center text-center shadow-inner animate-in slide-in-from-top-4 duration-300 ease-out">
                ...
                <div className="text-sm text-amber-800 font-medium leading-relaxed px-2 space-y-4">
                  ...
                  <div className="flex flex-col gap-2 items-center">
                    <div className="bg-white/50 py-2 px-4 rounded-xl border border-amber-200 inline-flex items-center gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300 delay-75 fill-mode-both">
                      <Calendar size={14} className="text-amber-700" />
                      <span className="font-black text-amber-900 uppercase text-[11px]">Pedidos aceitos até Quinta, 16/12</span>
                    </div>
                    <div className="bg-white/50 py-2 px-4 rounded-xl border border-amber-200 inline-flex items-center gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300 delay-150 fill-mode-both">
